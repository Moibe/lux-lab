<script lang="ts">
  ...
  const endpointPath = $derived(
    task === 'i2v' ? '/api/kling/v2.6/image-to-video-pro' : '/api/kling/v2.6/text-to-video-pro'
  );
  ...
  function addShot() { shots.push({ prompt: '', duration: 5 }); }
  ...
  async function handleSubmit(e: Event) {
    ...
    try {
      ...
      const voiceIds = voiceIdsRaw
        ...
        .map((s) => s.trim())
        .filter((s) => s.length > 0);
      if (voiceIds.length > 0) payload.voice_ids = voiceIds;
  ...
  {#if creationMode}
    <div class="form-body" transition:slide={{ duration: 280 }}>
      ...
      <Collapsible bind:open={advancedOpen} title="Avanzado">
        <div class="field">
          <label class="field-label" for="neg-prompt-26">Prompt negativo (qué evitar)</label>
          <Prompt
        ...
        />
        <div class="field">
          <label class="field-label" for="voice-ids">Voice IDs (opcional, separados por coma)</label>
          <input
            id="voice-ids"
            type="text"
            class="text-input"
            bind:value={voiceIdsRaw}
...
<style>
  ...
  .duration-row { margin-top: 1rem; display: flex; align-items: center; gap: 1rem; flex-wrap: wrap; }
  ...
  .duration-chip {
    ...
    color: var(--text-secondary); font-family: var(--font-sans); font-size: 0.88rem; font-weight: 500;
    cursor: pointer;
  ...
  .shot-card {
    ...
    background: var(--surface-pill); border: var(--border-subtle); border-radius: var(--radius-card);
  ...
  .add-shot {
    ...
    border-radius: var(--radius-card); color: var(--text-muted); font-family: var(--font-sans); font-size: 0.9rem;
  ...
  .text-input::placeholder { color: var(--text-muted); font-family: var(--font-sans); }
