<script lang="ts">
  ...
  import { V3_TIERS, V3_DEFAULT_TIER, type V3Tier } from '$lib/kling-v3-options';
  ...
  function readTierFromUrl(): V3Tier {
    const t = page.url.searchParams.get('tier');
    return V3_TIERS.some((x) => x.value === t) ? (t as V3Tier) : V3_DEFAULT_TIER;
  }
  ...
  let tier = $state<V3Tier>(readTierFromUrl());
  ...
  const endpointPath = $derived(
    task === 'i2v'
      ? `/api/kling/v3/image-to-video-${tier}`
      : `/api/kling/v3/text-to-video-${tier}`
  );
  ...
  // Pricing — todos los endpoints que este form puede invocar
  const ALL_ENDPOINTS = [
  ...
  const currentEndpointModel = $derived(
    creationMode === 'with-images'
      ? `fal-ai/kling-video/v3/${tier}/image-to-video`
...
>
  <div class="tier-row">
    <Dropdown
      bind:value={tier}
      options={V3_TIERS.map((t) => ({ value: t.value, label: t.label, hint: t.hint }))}
      label="Calidad:"
      disabled={isRunning}
    />
  </div>
