<script lang="ts">
  ...
  import { V25TURBO_TIERS, V25TURBO_DEFAULT_TIER, type V25TurboTier } from '$lib/kling-v2-5-turbo-options';
  ...
  function readTierFromUrl(): V25TurboTier {
    const t = page.url.searchParams.get('tier');
    return V25TURBO_TIERS.some((x) => x.value === t) ? (t as V25TurboTier) : V25TURBO_DEFAULT_TIER;
  }

  let tier = $state<V25TurboTier>(readTierFromUrl());
  ...
  // Standard tier sólo tiene i2v. Si el usuario está en text-only y cambia a standard,
  // forzamos modo con-imágenes para evitar enviar a un endpoint inexistente.
  $effect(() => {
    if (tier === 'standard' && creationMode === 'text-only') {
      creationMode = 'with-images';
    }
  });

  const supportsTextOnly = $derived(tier === 'pro');
  ...
  const endpointPath = $derived(
    task === 'i2v'
      ? `/api/kling/v2.5-turbo/image-to-video-${tier}`
      : `/api/kling/v2.5-turbo/text-to-video-pro`
  );
  ...
  const currentEndpointModel = $derived(
    creationMode === 'with-images'
      ? `fal-ai/kling-video/v2.5-turbo/${tier}/image-to-video`
      : `fal-ai/kling-video/v2.5-turbo/pro/text-to-video`
  );
...
>
  <div class="tier-row">
    <Dropdown
      bind:value={tier}
      options={V25TURBO_TIERS.map((t) => ({ value: t.value, label: t.label, hint: t.hint }))}
      label="Calidad:"
      disabled={isRunning}
    />
  </div>
  ...
  <div class="creation-mode-row">
    ...
    <div class="creation-mode-options">
      ...
      <button
        ...
        title={!supportsTextOnly ? 'Sólo Pro soporta text-to-video' : ''}
      >
        <span class="creation-mode-icon">✍️</span>
    ...
    {#if !supportsTextOnly}
      <span class="hint-text">Standard sólo soporta image-to-video. Cambia a Pro para texto.</span>
    {/if}
  </div>
  ...
  {#if creationMode}
    <div class="form-body" transition:slide={{ duration: 280 }}>
      ...
      <Collapsible bind:open={advancedOpen} title="Avanzado">
        <div class="field">
          <label class="field-label" for="neg-prompt-25">Prompt negativo (qué evitar)</label>
          <Prompt
